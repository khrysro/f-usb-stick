...
#from gi.repository import QObject, Gio, Polkit, Gtk, GLib, UDisks, #XApp
'''
try:
...
if Using_Unity:
    launcher = Unity.LauncherEntry.get_for_desktop_id ("formatstick.desktop")
'''
...
if Using_Unity:
    launcher = LauncherEntry.get_for_desktop_id ("formatstick.desktop")

APP = 'formatstick'
LOCALE_DIR = "/usr/share/khrysro/locale"
...
class FormatStick:
    def __init__(self, iso_path=None, usb_path=None, filesystem=None, mode=None, debug=False):
        ...
        # get glade tree
        self.gladefile = "/usr/share/formatstick/formatstick.ui"
...
class FormatStick:
        def __init__(self, iso_path=None, usb_path=None, filesystem=None, mode=None, debug=False ):
            super(FormatStick, self).__init__(*args)

            #loadUi('/usr/share/formatstick/formatstick.ui', self)
            loadUi('/home/khrys/desarrollo/Formatstick/share/formatstick/formatstick.ui', self)
            file = QFile("/home/khrys/desarrollo/Formatstick/share/formatstick/formatstick.ui")
            #file = QFile("/usr/share/formatstick/formatstick.ui")
            loader = loadUi(self)
...
'''
        ...
        def check_format_job(self):
            ...
            if self.process.returncode is None:
                ...
                QObject.idle_add(self.format_job_done, self.process.returncode)
                self.process = None
                return False
        ...
        def raw_format(self, usb_path, fstype, label):
            if os.geteuid() > 0:
                launcher='pkexec'
                self.process = Popen([launcher,'/usr/bin/python3', '-u', '/usr/lib/formatstick/raw_format.py',
                                      '-d',usb_path,'-f',fstype, '-l', label, '-u', str(os.geteuid()), '-g',
                                      str(os.getgid())], shell=False, stdout=PIPE,  preexec_fn=os.setsid)
            else:
                self.process = Popen(['/usr/bin/python3', '-u', '/usr/lib/formatstick/raw_format.py','-d',usb_path,
            ...
            QObject.timeout_add(500, self.check_format_job)
            ...
            if condition  is GLib.IO_IN:
                ...
                try:
                    ...
                    if progress > self.write_progress:
                        ...
                        QObject.idle_add(self.set_progress, size)
                        if Using_Unity:
        ...
        def check_write_job(self):
            ...
            if self.process.returncode is None:
                ...
                QObject.idle_add(self.write_job_done, self.process.returncode)
                self.process = None
                return False
        ...
        def raw_write(self, source, target):
            ...
            if os.geteuid() > 0:
                launcher='pkexec'
                self.process = Popen([launcher,'/usr/bin/python3', '-u', '/usr/lib/formatstick/raw_write.py','-s',source,'-t',target], shell=False, stdout=PIPE, preexec_fn=os.setsid)
            else:
                self.process = Popen(['/usr/bin/python3', '-u', '/usr/lib/formatstick/raw_write.py','-s',source,'-t',target], shell=False, stdout=PIPE, preexec_fn=os.setsid)

            self.write_progress = 0
            self.source_id = GLib.io_add_watch(self.process.stdout, GLib.IO_IN|GLib.IO_HUP, self.update_progress)
            QObject.timeout_add(500, self.check_write_job)
...
if __name__ == "__main__":
    ...
    def usage():
        print("Usage: formatstick [--debug] -m [format|iso]              : mode (format usb stick or burn iso image)")
        print("       formatstick [--debug] -m iso [-i|--iso] iso_path")
        print("       formatstick [--debug] -m format [-u|--usb] usb_device ")
    ...
    #FormatStick(iso_path, usb_path, filesystem, mode, debug)

    #start the main loop
    #mainloop = QObject.MainLoop()
    #mainloop.run()
